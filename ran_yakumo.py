import os, subprocess, platform, shutil, sqlite3 as sql
from pathlib import Path

bd_dir = str(Path(__file__).resolve().parent) + '/bd.db'

def createBD(bd=bd_dir):
    with sql.connect(bd) as conn:
        conn.execute("""CREATE TABLE IF NOT EXISTS Shortcuts(_id INTEGER PRIMARY KEY AUTOINCREMENT, key STRING NOT NULL, 
            value STRING NOT NULL UNIQUE, type STRING NOT NULL, param BOOLEAN NOT NULL DEFAULT 0, CHECK (type IN ('app', 'archive', 'site')))""")
...
def insertValue(key:str, value:str,  type:str, param:int=0, bd:str=bd_dir ):
    with sql.connect(bd) as conn: conn.execute(f"""INSERT INTO Shortcuts (key, value, type, param) VALUES (?,?,?,?)""", (key, value, type, param))
...
def getValue(key:str, bd:str=bd_dir)-> list[tuple[str, int,str]]:
    with sql.connect(bd) as conn: return conn.execute(f"SELECT value, param, type FROM Shortcuts WHERE key LIKE ?", (key,)).fetchall()

def getIDs(key:str,bd:str=bd_dir):
    with sql.connect(bd) as conn: rows = conn.execute("SELECT _id FROM Shortcuts WHERE key = ?",(key,)).fetchall()
    return [row[0] for row in rows]

def getColumns(bd:str=bd_dir)->tuple[int,str,str,str,int]:
    with sql.connect(bd) as conn: return tuple(c[1] for c in conn.execute(f'PRAGMA table_info(Shortcuts);').fetchall())

def updateValue(id_:int, key:str, value:str, type_:str, param:int, bd:str=bd_dir):
    with sql.connect(bd) as conn: conn.execute("UPDATE Shortcuts SET key=?, value=?, type=?, param=? WHERE _id =?",(key, value, type_, param, id_))

def deleteKey(key:str, bd:str=bd_dir):
    with sql.connect(bd) as conn: conn.execute("DELETE FROM Shortcuts WHERE key = ?",(key,))
...
def openDir(values:list[tuple[str,int, str]]):
    ...
    for v, arg, _ in values:
        if v.startswith('http') and arg:subprocess.Popen(['/usr/bin/brave', "--incognito", v])
            ...
            elif sys == 'Linux' : subprocess.Popen(["xdg-open", v])   # Ok
            else: print('Erro!')